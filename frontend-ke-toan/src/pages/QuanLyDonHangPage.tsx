...
import {
  ...
  FiCheck, FiX, FiEye, FiFileText,
} from 'react-icons/fi';
import {
  layDanhSachDonHang, duyetDonHang, tuChoiDonHang, xoaDonHang,
} from '../services/api';
import {
  DonHang, TRANG_THAI_DON_LABELS, ApiResponseWithPagination,
} from '../types';
import { useToast, usePagination } from '../hooks';
import { Pagination, Loading, EmptyState, ConfirmModal } from '../components/Common';
...
export default function QuanLyDonHangPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await layDanhSachDonHang(page, 20, trangThai || undefined, tuKhoa || undefined);
      setData(res);
    } catch {
  ...
  const kpiChoDuyet = data.data?.filter(d => d.trangThaiDon === 'cho_duyet').length || 0;
  const kpiDangXL = data.data?.filter(d =>
    ['da_duyet', 'dang_san_xuat', 'dang_giao', 'da_giao', 'nghiem_thu'].includes(d.trangThaiDon)
  ).length || 0;
  const kpiHoanThanh = data.data?.filter(d => d.trangThaiDon === 'da_thanh_toan').length || 0;
  ...
  const clearFilters = () => { setTuKhoa(''); setTrangThai(''); resetPage(); };
    ...
    <div>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <div className={styles.pageHeaderDesc}>Toàn quyền quản lý đơn hàng bê tông</div>
        </div>
        {canCreate && (
          <button className="btn btn-add" onClick={() => navigate('/quan-ly/don-hang/tao')}>
            <FiPlus /> Tạo đơn hàng
          </button>
        )}
      </div>
      ...
      <div className={styles.kpiRow}>
        ...
        <div className={styles.kpiItem}>
          ...
          <div className={styles.kpiValue} style={{ color: 'var(--color-warning)' }}>{kpiChoDuyet}</div>
        </div>
        <div className={styles.kpiItem}>
          <div className={styles.kpiLabel}>Đang xử lý</div>
          <div className={styles.kpiValue} style={{ color: 'var(--color-purple)' }}>{kpiDangXL}</div>
        </div>
        <div className={styles.kpiItem}>
          <div className={styles.kpiLabel}>Hoàn thành</div>
          <div className={styles.kpiValue} style={{ color: 'var(--color-success)' }}>{kpiHoanThanh}</div>
        </div>
      </div>
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={{ minWidth: 160 }}>Địa chỉ</th>
                  <th style={{ minWidth: 90 }}>Mác BT</th>
                  <th style={{ minWidth: 90, textAlign: 'right' }}>Khối lượng</th>
                  <th style={{ minWidth: 120, textAlign: 'right' }}>Thành tiền</th>
                  <th style={{ minWidth: 120 }}>Trạng thái</th>
                  <th style={{ minWidth: 100 }}>Ngày tạo</th>
              ...
              <tbody>
                {data.data?.map((dh) => (
                  <tr key={dh.id}>
                    <td><strong className={styles.tableCode}>{dh.maDonHang}</strong></td>
                    <td>
                      <div className={styles.tableName}>{dh.tenKhachHang}</div>
                      <div className={styles.tableSub}>{dh.soDienThoai}</div>
                    </td>
                    <td className={styles.tableAddress}>{dh.diaChiNhan}</td>
                    <td>
                      <span className={styles.tableMac}>{dh.tenMacBeTong}</span>
                    </td>
                    <td className={styles.tableRight}>{dh.khoiLuongDat} m³</td>
                    <td className={styles.tableRight}><strong>{formatCurrency(dh.thanhTien || 0)}</strong></td>
                    <td>
                      <span
                    ...
                    <td className={styles.tableDate}>{formatDate(dh.ngayTaoDon)}</td>
                    <td>
                      <div className={styles.rowActions}>
                        ...
                        {/* Duyệt / Từ chối — chỉ khi chờ duyệt + có quyền */}
                        ...
                        {/* Sửa */}
                        {canEdit && ['cho_duyet', 'da_duyet'].includes(dh.trangThaiDon) && (
                          <button
                        ...
                        {/* Xóa */}
                        {canDelete && ['cho_duyet', 'tu_choi'].includes(dh.trangThaiDon) && (
                          <button
