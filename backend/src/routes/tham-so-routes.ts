...
router.post('/mac-be-tong', authMiddleware, requireRole('admin', 'giam_doc_kinh_doanh', 'dieu_phoi', 'sale'), async (req: AuthRequest, res: Response<ApiResponse>) => {
  try {
...
router.put('/mac-be-tong/:id', authMiddleware, requireRole('admin', 'giam_doc_kinh_doanh', 'dieu_phoi', 'sale'), async (req: AuthRequest, res: Response<ApiResponse>) => {
  try {
    const id = parseInt(req.params.id, 10);
